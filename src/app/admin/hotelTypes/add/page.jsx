...
function Page() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formdata.label || !formdata.thumb) {
        ...
        formData.append('label', formdata.label);
        formData.append('thumb', formdata.thumb);

        console.log("FormData: ", formData);

        try {
            const response = await axios.post(`http://localhost:5000/api/hotelTypes`, formData, {
                headers: {
            ...
            if (response.status == 201) {
                Swal.fire({
                    ...
                    router.push("./../../../../admin/hotelTypes/list");
                });
            } else {
                Swal.fire({
                    icon: "error",
                    ...
                    <div className='p-1'>
                        <div className='mx-auto max-w-[600px] mt-12'>
                            <form onSubmit={handleSubmit} name="employeeForm" id="employeeForm" className="bg-white shadow-md rounded px-12 pb-4 mb-4 py-2 mt-2"
                                ...
                                <h1 className='text-2xl font-medium text-center pb-7 text-gray-800 '>Add New Hotel Type</h1>
                                ...
                                <div className="flex items-center justify-end mt-8">
                                    <button
                                        ...
                                        disabled={loading}
                                    >
                                        {loading ? 'Loading...' : 'Add Hotel Type'}
                                    </button>
                                </div>
                            </form>
                        </div>
