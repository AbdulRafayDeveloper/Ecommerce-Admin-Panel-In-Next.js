...
function Page() {
    ...
    const [filteredroles, setfilteredroles] = useState([]); // Initialize with an empty array
    const perPage = 10 // Number of items per page

    useEffect(() => {
        const fetchData = async () => {
            await axios.get(`http://localhost:5000/api/roles`)
                .then((result) => {
    ...
    const displayedEmployee = filteredroles.slice(startIndex, endIndex);

    const handleFilter = (e) => {
        const searchText = e.target.value.toLowerCase();

        if (searchText.trim() === '') {
            setfilteredroles(roles); // Reset filtered data to all data
        } else {
        ...
        <div className="flex flex-col md:flex-row h-screen">
            ...
            <div className="flex-1 overflow-auto bg-gray-100">
                ...
                <div className='p-4 bg-white mx-5 mt-3 rounded-lg shadow-lg'>
                    <div>
                        <h2 className="text-2xl font-medium mb-3 text-black">Roles List</h2>
                        <div className='flex justify-between items-center mb-4'>
                            <div className='flex'>
                                <Link href="../../../admin/roles/add" className='text-sm w-28 h-8 flex items-center justify-center border border-blue-600 bg-white mr-3 rounded-xl transition duration-300 ease-in-out transform hover:scale-105 shadow-sm' passHref>
                            ...
                            <table className="w-full bg-white rounded-lg shadow-lg overflow-hidden">
                                <thead className="bg-blue-400">
                                    <tr>
                                        ...
                                        <th className="p-2 border border-gray-300 text-white" style={{ width: "40%" }}>Name</th>
