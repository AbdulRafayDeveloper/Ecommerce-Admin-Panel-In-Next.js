...
function Page() {
    ...
    const [hotelInfrastructures, sethotelInfrastructures] = useState([]);
    const [currentPage, setCurrentPage] = useState(0); // Current page number
    const [filteredhotelInfrastructures, setfilteredhotelInfrastructures] = useState([]); // Initialize with an empty array
    const perPage = 12 // Number of items per page

    useEffect(() => {
        const fetchData = async () => {
            await axios.get(`http://localhost:5000/api/hotelInfrastructure`)
                .then((result) => {
                    console.log("result.data: ", result.data);
                    if (Array.isArray(result.data)) {
                        console.log("result.data.data: ", result.data);
                        sethotelInfrastructures(result.data);
                        setfilteredhotelInfrastructures(result.data);
                    } else {
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = await axios.delete(`http://localhost:5000/api/hotelInfrastructure/${id}`);
    ...
    const displayedEmployee = filteredhotelInfrastructures.slice(startIndex, endIndex);

    const handleFilter = (e) => {
        const searchText = e.target.value.toLowerCase();

        if (searchText.trim() === '') {
            setfilteredhotelInfrastructures(hotelInfrastructures); // Reset filtered data to all data
        } else {
            const filteredData = hotelInfrastructures.filter((item) =>
                Object.values(item).some((value) =>
                    String(value).toLowerCase().includes(searchText)
                )
            );
            setfilteredhotelInfrastructures(filteredData);
        }
        ...
        <div className="flex flex-col md:flex-row h-screen">
            ...
            <div className="flex-1 overflow-auto bg-gray-100">
                ...
                <div className='p-4 bg-white mx-5 mt-3 rounded-lg shadow-lg'>
                    <div>
                        <h2 className="text-2xl font-medium mb-3 text-black">Hotel Infrastructures List</h2>
                        <div className='flex justify-between items-center mb-4'>
                            <div className='flex'>
                                <Link href="../../../admin/hotelInfrastructure/add" className='text-sm w-28 h-8 flex items-center justify-center border border-blue-600 bg-white mr-3 rounded-xl transition duration-300 ease-in-out transform hover:scale-105 shadow-sm' passHref>
                        ...
                        <div className="flex justify-center mt-4">
                            <ReactPaginate
                                ...
                                pageCount={Math.ceil(hotelInfrastructures.length / perPage)}
