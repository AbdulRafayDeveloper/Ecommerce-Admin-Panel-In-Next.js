...
function Page() {
    ...
    const [filteredhotelServices, setfilteredhotelServices] = useState([]); // Initialize with an empty array
            ...
            await axios.get(`http://localhost:5000/api/hotelService`)
                .then((result) => {
                    ...
                    if (Array.isArray(result.data)) {
                        ...
                        setfilteredhotelServices(result.data);
                    } else {
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = await axios.delete(`http://localhost:5000/api/hotelService/${id}`);
    ...
    const displayedEmployee = filteredhotelServices.slice(startIndex, endIndex);

    const handleFilter = (e) => {
        const searchText = e.target.value.toLowerCase();

        if (searchText.trim() === '') {
            setfilteredhotelServices(hotelServices); // Reset filtered data to all data
        } else {
            const filteredData = hotelServices.filter((item) =>
                Object.values(item).some((value) =>
                    String(value).toLowerCase().includes(searchText)
                )
            );
            setfilteredhotelServices(filteredData);
        }
        ...
        <div className="flex flex-col md:flex-row h-screen">
            ...
            <div className="flex-1 overflow-auto bg-gray-100">
                ...
                <div className='p-4 bg-white mx-5 mt-3 rounded-lg shadow-lg'>
                    <div>
                        <h2 className="text-2xl font-medium mb-3 text-black">Hotel Services List</h2>
                        <div className='flex justify-between items-center mb-4'>
                            <div className='flex'>
                                <Link href="../../../admin/hotelService/add" className='text-sm w-28 h-8 flex items-center justify-center border border-blue-600 bg-white mr-3 rounded-xl transition duration-300 ease-in-out transform hover:scale-105 shadow-sm' passHref>
                        ...
                        <div className="flex justify-center mt-4">
                            <ReactPaginate
                                ...
                                pageCount={Math.ceil(hotelServices.length / perPage)}
