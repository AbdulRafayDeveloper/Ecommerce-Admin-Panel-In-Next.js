...
function Page() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            await axios.get(`http://localhost:5000/api/meta`)
                .then((result) => {
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = await axios.delete(`http://localhost:5000/api/meta/${id}`);
        ...
        <div className="flex flex-col md:flex-row h-screen">
            ...
            <div className="flex-1 overflow-auto bg-gray-100">
                ...
                <div className='p-4 bg-white mx-5 mt-3 rounded-lg shadow-lg'>
                    <div>
                        <h2 className="text-2xl font-medium mb-3 text-black">Meta List</h2>
                            ...
                            <table className="w-full bg-white rounded-lg shadow-lg overflow-hidden">
                                <thead className="bg-blue-400">
                                    <tr>
                                        ...
                                        <th className="p-2 border border-gray-300 text-white" style={{ width: "16%" }}>Title</th>
                                        <th className="p-2 border border-gray-300 text-white" style={{ width: "16%" }}>Description</th>
                                        <th className="p-2 border border-gray-300 text-white" style={{ width: "16%" }}>Page</th>
                                        <th className="p-2 border border-gray-300 text-white" style={{ width: "16%" }}>Url</th>
                                    ...
                                    {displayedEmployee && displayedEmployee.length > 0 ? (
                                        displayedEmployee.map((element, index) => (
                                            <tr className="hover:bg-gray-100">
                                                ...
                                                <td className="text-center border border-gray-300 text-gray-900 p-1">{element.page}</td>
                                                <td className="text-center border border-gray-300 text-gray-900 p-1">{element.url}</td>
                                                <td className="text-center border border-gray-300 p-1">
                                                    <div className='flex space-x-3 justify-center'>
                                                        <Link href={`../../../admin/meta/update/${element._id}`}>
