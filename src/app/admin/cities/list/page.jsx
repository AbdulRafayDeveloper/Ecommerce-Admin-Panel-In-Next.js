...
function Page() {
    ...
    const [filteredHotelTypes, setfilteredHotelTypes] = useState([]);
    const perPage = 12
            ...
            await axios.get(`http://localhost:5000/api/cities`)
                .then((result) => {
                    ...
                    if (Array.isArray(result.data.data)) {
                        ...
                        sethotelTypes(result.data.data);
                        setfilteredHotelTypes(result.data.data);
                    } else {
    ...
    const displayedEmployee = filteredHotelTypes.slice(startIndex, endIndex);

    const handleFilter = (e) => {
        const searchText = e.target.value.toLowerCase();

        if (searchText.trim() === '') {
            setfilteredHotelTypes(hotelTypes); // Reset filtered data to all data
        } else {
            const filteredData = hotelTypes.filter((item) =>
                Object.values(item).some((value) =>
                    String(value).toLowerCase().includes(searchText)
                )
            );
            setfilteredHotelTypes(filteredData);
        }
        ...
        <div className="flex flex-col md:flex-row h-screen">
            ...
            <div className="flex-1 overflow-auto bg-gray-100">
                ...
                <div className='p-4 bg-white mx-5 mt-3 rounded-lg shadow-lg'>
                    <div>
                        <h2 className="text-2xl font-medium mb-3 text-black">Cities List</h2>
                        <div className='flex justify-between items-center mb-4'>
                            <div className='flex'>
                                <Link href="../../../admin/cities/add" className='text-sm w-28 h-8 flex items-center justify-center border border-blue-600 bg-white mr-3 rounded-xl transition duration-300 ease-in-out transform hover:scale-105 shadow-sm' passHref>
                            ...
                            <table className="w-full bg-white rounded-lg shadow-lg overflow-hidden">
                                <thead className="bg-blue-400">
                                    <tr>
                                        ...
                                        <th className="p-2 border border-gray-300 text-white" style={{ width: "16%" }}>City Name</th>
                                        <th className="p-2 border border-gray-300 text-white" style={{ width: "16%" }}>Country</th>
                                        <th className="p-2 border border-gray-300 text-white" style={{ width: "16%" }}>Region</th>
                                        <th className="p-2 border border-gray-300 text-white" style={{ width: "16%" }}>Subdomain</th>
                                        <th className="p-2 border border-gray-300 text-white" style={{ width: "16%" }}>Thumbnail</th>
                                    ...
                                    {displayedEmployee && displayedEmployee.length > 0 ? (
                                        displayedEmployee.map((element, index) => (
                                            <tr className="hover:bg-gray-100">
                                                ...
                                                <td className="text-center border border-gray-300 text-gray-900 p-1">{element.cityName}</td>
                                                <td className="text-center border border-gray-300 text-gray-900 p-1">{element.country}</td>
                                                <td className="text-center border border-gray-300 text-gray-900 p-1">{element.region}</td>
                                                <td className="text-center border border-gray-300 text-gray-900 p-1">{element.subdomain}</td>
                                                <td className="text-center border border-gray-300 p-1 text-gray-900">
                                                    <Image src={`${element.thumbnail}`} width={50} height={50} alt="City Picture" className='mx-auto w-8 h-8 object-cover' />
                                                </td>
                                        ...
                                        <tr>
                                            <td colSpan="7" className="text-center border border-gray-300 text-gray-900 p-1">
                                                No records found
                                            </td>
                                        </tr>
                                    )}
                        ...
                        <div className="flex justify-center mt-4">
                            <ReactPaginate
                                ...
                                pageCount={Math.ceil(hotelTypes.length / perPage)}
